...
- **Un-acked sends are a shared resource, not a private mess.** The stack runs message-worker with `NUM_WORKERS: '1'`, so a send that no test polls and acks blocks the single worker goroutine for facebot's full 10s timeout. In a `mocha.parallel` block that starves every *other* test too, and the symptom is a wave of unrelated 45s timeouts rather than a failure in the test that caused it. A test that drives a form must consume every message that form produces — including the trailing question it does not care about.
- **`MEDIA_HANDLE_USE` must be on in the test stack.** message-worker's media handle layer (`planning/media-abstraction.md`) defaults to **off** in `config.go` because it ships dark in production; `stack.ts` sets `MEDIA_HANDLE_USE: 'true'` (plus `MEDIA_HANDLE_MARGIN`). Without it, every media resolution test silently passes by sending everything by URL — a handle miss is not an error, it is the designed fallback, so the suite would look green while resolution was never exercised.
...
- **Media resolution end-to-end** (`planning/media-abstraction.md` §8.3, §10 section 4): seven tests assert the *shape* of the outbound media payload that reached facebot — which key is present and which is absent — against handle rows seeded in CockroachDB. Messenger cases sit in `Basic Functionality`, WhatsApp cases in `WhatsApp E2E`. They cover the legacy `attachment_id` passthrough (rule 1, ~100% of live media traffic), an asset URL with a live handle (sends by id), an asset URL with no handle (degrades to URL), a third-party URL (never resolved), WhatsApp `{id}` vs `{link}`, and **handle reuse** — the same asset sent twice in one flow must send by id both times, without which every send could be a silent URL fallback and the suite would still pass. Deliberately **no MinIO and no dashboard-server** in this stack: the worker never reads bytes, it puts a URL string in the payload and facebot never fetches it. Upload/fan-out is covered by dashboard-server's own suite.
...
- **`facebot/receiver/index.js`**: Facebot mock — queues outbound messages for `flowMaster` to poll, answers `POST /me/pass_thread_control` for handoff tests, and implements media upload endpoints (`POST /me/message_attachments` for Messenger, `POST /:phoneNumberId/media` for WhatsApp) plus inspection routes (`GET /uploads`, `POST /uploads/reset`) for testing media fan-out
