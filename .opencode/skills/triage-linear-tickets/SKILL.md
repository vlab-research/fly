...
The Linear MCP is configured in `opencode.json` as a remote MCP server (`https://mcp.linear.app/mcp`). Use the MCP tools to query and mutate Linear. The key operations:
