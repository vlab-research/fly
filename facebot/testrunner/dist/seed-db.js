...
async function insertSurvey(pool, filename, body, userid, shortcode) {
...
function readForm(form) {
    return [form, fs_1.default.readFileSync(`forms/${form}`, 'utf8')];
}
async function seed(chatbase) {
    ...
    const inserts = fs_1.default.readdirSync('forms')
        .map((form) => readForm(form))
